...
<body>
  <div class="container">
        ...
        <div class="right-div" style="padding: 5px;">
          <div style="display: inline-block; width: 90%;">
            <table class="table-title">
              ...
              <tr>
                <td>{{ date('d', strtotime($ingreso['fecha_solicitud'])) }}</td>
                <td>{{ date('m', strtotime($ingreso['fecha_solicitud'])) }}</td>
                <td>{{ date('Y', strtotime($ingreso['fecha_solicitud'])) }}</td>
              </tr>
            </table>
          </div>
          <div class="right-div">
            <div style="display: inline-block; width: 90%;">
              <table class="table-title">
                <tr>
                  <td><span style="font-size: 20px; font-weight: bold;">N° {{ $ingreso['numero'] }}</span></td>
                </tr>
              </table>
            </div>
          </div>
    ...
    <div style="width: 620px; margin: 2px auto; margin-bottom: 1; text-align: center;">
      <span style="font-size: 30px; font-weight: bold;">SOLICITUD DE INGRESO DE CAJAS</span>
    </div>
    <div class="row" style="color: #000000;">
      <div style="border: 2px solid #707070; border-radius: 7px;">
        <div style="float: right; margin: 8px; width:200px; color: #555;"><b>Código: {{ $ingreso['codigo'] }}</b></div>
        <div style="margin: 2px 2px 2px 10px; color: #555;"><strong>Agencia / Empresa :</strong> {{ $ingreso['agencia'] }}</div>
        <div style="margin: 2px 2px 2px 10px; color: #555;"><strong>Fecha Solicitud :</strong> {{ date('d/m/Y', strtotime($ingreso['fecha_solicitud'])) }} &nbsp;&nbsp;<strong>Fecha Aprobación :</strong> {{ $ingreso['fecha_aprobacion'] }} &nbsp;&nbsp;<strong>Fecha Entrega :</strong> {{ $ingreso['fecha_entrega'] }} </div>
        <div style="margin: 2px 2px 2px 10px; color: #555;"><strong>Estado :</strong> {{ $ingreso['estado'] }}</div>
        <div style="margin: 2px 2px 2px 10px; color: #555;"><strong>Observaciones :</strong> {{ $ingreso['observacion'] }}</div>
      </div>
    </div>
    ...
    <span style="color: #2A2A2A; font-weight: bold; font-size: 12px;">Total Cajas: {{ $ingreso['total_cajas'] }}</span>
