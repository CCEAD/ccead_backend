      ...
      <div class="leftdivcontainer clearfix">
        <div class="leftdiv">
          <div style="border: 2px solid #287729; border-radius: 7px; text-align: left; width: 60%; padding: 10px;">
            <span style="color: #000; font-weight: bold; font-size: 15px; letter-spacing: 1px;">{{ $salidas[0]['agencia'] }}</span>
          </div>
        </div>
        <div class="leftdiv" style="margin-top: 0;">
          <div style="color: #000; font-size: 22px; font-weight: bold; text-decoration: underline;">LISTA GENERAL DE SALIDAS ALMACÉN</div>
        </div>
        <div class="leftdiv">
      ...
      <tbody>
        @foreach($salidas as $salida)
          <tr style="page-break-inside: avoid;">
            <td class="text-content">{{ $salida['codigo'] }}</td>
            <td class="text-content">{{ $salida['fecha_solicitud'] }}</td>
            <td class="text-content">{{ $salida['fecha_aprobacion'] }}</td>
            <td class="text-content">{{ $salida['fecha_entrega'] }}</td>
            <td class="text-content">{{ $salida['estado'] }}</td>
            <td class="text-content">{{ $salida['total_cajas'] }}</td>
          </tr>
        @endforeach
      </tbody>
    </table>
